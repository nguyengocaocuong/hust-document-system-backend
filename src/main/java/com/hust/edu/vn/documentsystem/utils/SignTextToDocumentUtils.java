...
@Component
public class SignTextToDocumentUtils {
    ...
    private byte[] signToPDF(InputStream inputStream) throws IOException {
    ...
    private byte[] signToImage(InputStream inputStream) throws IOException {
        BufferedImage image = ImageIO.read(inputStream);
        Graphics2D graphics2D = image.createGraphics();
        Font font = new Font("Arial", Font.BOLD, 48);
        Color color = Color.RED;
        FontMetrics fontMetrics = graphics2D.getFontMetrics(font);
        int textWidth = fontMetrics.stringWidth("HUST DOCUMENT SYSTEM");
